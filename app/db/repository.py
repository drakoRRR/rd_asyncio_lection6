...
async def save_cve_to_db(session: AsyncSession, data_list: list):
    ...
    for data in data_list:
        ...
        title = cna.get('descriptions', [{}])[0].get('value', "Unknown Title")  # Используем значение по умолчанию
        description = cna.get('descriptions', [{}])[0].get('value', "No description available")  # Используем значение по умолчанию
        problem_types = ", ".join(
            [
